...
public class CryptoUtil {
	...
	public static String encryptAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		...
		byte[] keyBytes = new byte[16];  // 크기가 16인 바이트 배열을 만든다.
		byte[] b = key.getBytes("UTF-8");	// key 를 문자열로 
		int len = keyBytes.length;
		
		System.arraycopy(b, 0, keyBytes, 0, len);	
		// b : 복사하고자 하는 소스(원본)
		// 0 : 위의 원본 소스에서 어느 부분 부터 읽어올지 위치 정해줌
		// keyByte : 복사할 소스
		// 0 : 복사본에서 자료를 받을때 어느 부분부터 쓸 것인지 시작 위치를 정해줌.
		// len : 원본에서 복사본으로 데이터를 읽어서 쓸 데이터의 길이.
		...
		// 옵션 종류에 맞게 초기화 한다.
		// 옵션 종류 : ENCRYPT_MODE(암호화모드), DECRY
		// 초기화 벡터(Initial Vector, IV)
		// ==> 암호문이 패턴화 되지 않도록 사용하는 데이터
		//		 첫 블록을 암호회 할 대 사용하는 값
	...
	// 복호화 할 문자열 decoding해서 암호화된 byte배열을 구한다.
	byte[] byteStr = Base64.getDecoder().decode(str);
	
	// 복호화 도니 데이터를 문자열로 변환하여 반환한다.
